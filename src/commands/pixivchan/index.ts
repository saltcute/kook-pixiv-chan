import * as pixiv from './pixiv/common';
import * as pixivadmin from './pixiv/admin/common';
import config from "configs/config";
import { bot } from 'init/client';
import { pixivMenu } from './pixiv/pixiv.menu';
import { pixivAdminMenu } from './pixiv/admin/pixivadmin.menu';
import axios from 'axios';
import auth from 'configs/auth';
import luxon from 'luxon';
import { author } from './pixiv/pixiv.author.app';
import { detail } from './pixiv/pixiv.detail.app';
import { gui } from './pixiv/pixiv.gui.app';
import { random } from './pixiv/pixiv.random.app';
import { top } from './pixiv/pixiv.top.app';
...
bot.message.on('allTextMessages', (event) => {
    switch (true) {
        ...
        case /^查询画师 ?([0-9]+)$/.test(event.content): {
            ...
            author.exec([matches[0]], event, bot);
            break;
        };
        case /^查询(?:图片|插画|涩涩|(?:[蛇色瑟涩]|se|she)图)? ?([0-9]+)$/.test(event.content): {
            ...
            detail.exec([matches[0]], event, bot);
            break;
        }
    }
})
...
bot.message.on("buttonClicked", async (event) => {
    try {
        ...
        try {
            ...
            const func: (e: typeof event, action: string[], data: any) => Promise<any> = require(upath.join(__dirname, 'button', ...action, 'index')).default;
